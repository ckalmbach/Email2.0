...
mail.login('cognisens.python.parser@gmail.com','neurotracker')
...
for x in range(i):
    ...
    if registration_declined!=-1:
        ...
        if (db_version == "2.5.5b"):
            ...
            lic_id=raw_email_string[location1+1:location2]
        else:
            SECOND_DELIMITER = " "
            location2=raw_email_string.find(SECOND_DELIMITER,location1+len(FIRST_DELIMITER),len(raw_email_string))
            lic_id=raw_email_string[location1+len(FIRST_DELIMITER)+2:location2]
        print lic_id;
